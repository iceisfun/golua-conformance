...
| `compilefuzz/` | Differential **compiler-limit / deep-nesting** grinder (compile-time): sweeps each compiler limit (locals/upvalues/params/args/fields/and-or/returns/multi-assign) and structural-nesting axis (parens/index-field-chains/blocks/functions/jumps) across the boundary, and diffs what `load()` accepts/rejects + the error core vs the reference. Asserts deep/large source only ever yields a *catchable* error (no Go fatal-stack/OOM host crash). Found the O(n²) too-many-locals compile DoS and the index/field/and-or chain register over-allocation. |
| `pairsfuzz/` | Differential **+** invariant fuzzer for `pairs`/`next`/table iteration. Since iteration *order* is unspecified, it compares only order-independent facts: the canonical **set** of visited (k,v) pairs (type-tagged, sorted — so different visit orders still compare equal) and oracle-free invariants (no duplicate/skipped keys, `pairs`↔`next` agreement, mutation-during-traversal termination, `next` error edges). Targets the real bug classes — key normalization (`1`/`1.0`/`"1"`, `-0.0`, `2^53`), completeness across array↔hash transitions — without ever flagging order. |
| `rubegoldberg/` | **Semantic** program generator: composes individually-correct stages (closures, coroutines, iterators, patterns, stateful/self-modifying metatables, sieves, sorts, recursion, varargs, deep shadowing) into deep *valid, deterministic* Lua "machines" and diffs golua vs the reference. Complexity is emergent from composition, not random syntax. Enforces determinism (integer spine; no floats/random/os/io/pairs-order/table-tostring), guards non-determinism by running the reference twice, and auto-reduces any differential failure to a minimal reproducer by delta-debugging stages. |
| `bytecodefuzz/` | **Oracle-free** VM-robustness fuzzer that drives golua with malformed *bytecode* (`load(...,"b")`). Dumps a diverse proto corpus via `string.dump`, locates each instruction array (replaying the undump parse) and applies operand-targeted mutations (register/constant/Bx/MaxStack blow-ups) + broad flips/truncation, then executes each under ulimit+timeout. Flags only golua-specific uncatchable escapes (Go slice-OOB panic / SIGSEGV from an unchecked register/constant index) — crafted-bytecode hangs are expected (no Lua ships a verifier). Complements the `undump` allocation-bound fix. |
...
| `debugfuzz/` | Differential grinder for the **debug library** and the **source line-info model** (golua's AST/one-pass compiler + VM-frame model vs reference's bytecode + C-stack model). Three channels: LINEHOOK (visited-line trace under a line hook over a head×tail statement matrix), ERRLINE (user-visible error line + traceback on faulting discharge instructions), DEBUGAPI (getinfo/getlocal/upvalue/traceback/… over C/Lua/main/vararg/tail-call/coroutine frames). Found the ADDI/SUBI line-relabel bug. |
