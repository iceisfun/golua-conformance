#!/usr/bin/env python3
"""Differential + invariant grinder for string.format (printf-style).
...
(the %q round-trip). Only divergences are kept (written to corpus/). A clean run
leaves corpus empty.
...
def _has_ptr(fmt):
    # crude but sufficient: a %p conversion (skip %%p literal-percent runs)
    i = 0
    while i < len(fmt):
        if fmt[i] == "%":
            j = i + 1
...
def make_cases(fmts, seed, illegal=False):
    rng = valmod.random.Random(seed)
    cases = []
    for k, fmt in enumerate(fmts):
        tuples = valmod.value_tuples(fmt, rng, illegal=illegal)
        if not tuples:
            tuples = [[]]
        hp = _has_ptr(fmt)
...
def process(fmts, label, seed, illegal=False):
    cases = make_cases(fmts, seed, illegal=illegal)
...
def main():
    ...
    ap.add_argument("--illegal", action="store_true", help="run the illegal/error tier")
    ...
    def run(fmts, label, illegal=False):
    ...
    if do_illegal:
        all_leads += run(grammar.illegal(), "illegal", illegal=True)
    if args.tier3:
