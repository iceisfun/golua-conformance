#!/usr/bin/env python3
"""Differential + invariant grinder for string.pack/unpack/packsize.

Runs each generated case under BOTH golua and reference lua5.5.0, diffs their
canonical output line-by-line, and inspects golua's oracle-free invariant lines.
Only divergences are kept (written to corpus/). A clean run leaves corpus empty.

Usage:
  python3 run.py --tier 0
  python3 run.py --tier 1 --tier 2
  python3 run.py --tier3 200000 --seed 1
  python3 run.py --all
Env:
...
def make_cases(fmts, seed):
    rng = valmod.random.Random(seed)
    cases = []
    for k, fmt in enumerate(fmts):
        tuples = valmod.value_tuples(fmt, rng)
        if not tuples:
            tuples = [[]]
        for j, vals in enumerate(tuples):
            cases.append({"id": "c%d_%d" % (k, j), "fmt": fmt, "vals": vals})
    return cases


def diff_outputs(golua_out, ref_out):
    """Return list of (golua_line, ref_line) where they differ, keyed by case+op."""
    def index(text):
        idx = {}
    ...
    for key in sorted(set(gi) | set(ri)):
        # INV.* lines are golua-only oracle checks; ref also emits them but they
        # must read 'true' on both — handled separately. Still diff them.
...
def run_batch(cases, label, batch_no, leads):
    ...
    try:
        ...
        for key, g, r in diff_outputs(g_out, r_out):
            if key[0] == "INV":
                continue  # invariant lines handled above; diffs there are benign noise
...
def process(fmts, label, seed):
    cases = make_cases(fmts, seed)
    print("%s: %d formats -> %d cases" % (label, len(fmts), len(cases)))
    leads = []
...
def main():
    ...
    all_leads = []
    total_cases = 0
    # Tag each phase label with the seed so every lead line records its origin.
    def run(fmts, label):
        nonlocal total_cases
        leads, ncases = process(fmts, "%s.s%d" % (label, args.seed), args.seed)
        total_cases += ncases
        return leads
