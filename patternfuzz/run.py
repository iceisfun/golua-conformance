#!/usr/bin/env python3
"""Differential + invariant grinder for Lua string pattern matching.

Runs each generated (pattern, subject) case under BOTH golua and reference
lua5.5.0 across string.find / match / gmatch / gsub, diffs their canonical
output line-by-line, and inspects golua's oracle-free invariant lines. Only
divergences are kept (written to corpus/). A clean run leaves corpus empty.

Usage:
  python3 run.py --tier 0
  python3 run.py --tier 1 --tier 2 --illegal
...
# GOLUA_REPO defaults to a sibling directory named "golua"; override if yours
# lives elsewhere (e.g. GOLUA_REPO=~/src/golua).
WORK = os.path.dirname(os.path.dirname(HERE))   # .../golua-conformance -> parent
...
TIMEOUT_S = 30
BATCH = 300                   # cases per driver invocation (localizes crashes)
...
def make_cases(patterns, subjects):
    """Cross every pattern with every subject -> one case each."""
    cases = []
    for k, pat in enumerate(patterns):
        pb = pat.encode("latin-1") if isinstance(pat, str) else pat
        for j, sub in enumerate(subjects):
            cases.append({"id": "c%d_%d" % (k, j), "pat": pb, "sub": sub})
    return cases
...
def diff_outputs(golua_out, ref_out):
    ...
    Key is (op, id); INV lines are golua-only oracle checks handled separately.
    """
    def index(text):
        idx = {}
...
def invariant_failures(golua_out):
    ...
    for line in golua_out.splitlines():
        cols = line.split("\t")
        if cols[0] == "INV" and len(cols) >= 4 and cols[3] not in ("true",) \
                and not cols[3].startswith("len="):
            fails.append(line)
    return fails
...
def run_batch(cases, label, batch_no, leads):
    ...
    try:
        ...
        for line in invariant_failures(g_out):
            leads.append(("INVARIANT", label, line, "", ""))

        for key, g, r in diff_outputs(g_out, r_out):
            if key[0] == "INV":
                continue  # invariant lines handled above
            leads.append(("DIFF", label, "\t".join(key), "golua: " + g, "ref:   " + r))
    finally:
...
def _dump_panic(label, batch_no, err, cases, path):
    ...
    with open(p, "w") as f:
        ...
        f.write("\n\n--- cases ---\n")
        for c in cases:
            f.write("%s  pat=%r  sub=%r\n" % (c["id"], c["pat"], c["sub"]))


def write_corpus(leads, seed):
    """Write lead files, namespaced by seed so a multi-seed loop never clobbers."""
...
def write_report(seed, ncases, leads):
    """Append one durable summary line per invocation — clean seeds included."""
    os.makedirs(CORPUS, exist_ok=True)
...
def main():
    ...
    def run(patterns, subjects, label):
        nonlocal total_cases
        cases = make_cases(patterns, subjects)
        total_cases += len(cases)
        return process(cases, "%s.s%d" % (label, args.seed))

    if 0 in tiers:
        all_leads += run(grammar.tier0(), valmod.FIXED_SUBJECTS, "tier0")
    if 1 in tiers:
        all_leads += run(grammar.tier1(), valmod.FIXED_SUBJECTS, "tier1")
    if 2 in tiers:
    ...
    if args.tier3:
        ...
        pats = grammar.tier3(args.seed, args.tier3)
        subs = valmod.CORE_SUBJECTS + valmod.random_subjects(rng, 8)
        all_leads += run(pats, subs, "tier3")
