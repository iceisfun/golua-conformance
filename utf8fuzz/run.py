#!/usr/bin/env python3
"""Differential + invariant grinder for the utf8 library.
...
def diff_outputs(golua_out, ref_out):
    ...
    Lines are keyed by (op, tag) — the first two tab columns.
    """
    def index(text):
        idx = {}
...
def invariant_failures(out):
    """INV lines whose value column is not literally 'true'."""
    fails = []
...
def run_batch(cases, label, batch_no, leads):
    ...
    try:
        ...
        # Differential invariant check: an INV line must read 'true' under
        # golua. (Reference also emits INV lines; the diff layer below catches
        # any cross-interpreter INV disagreement too.)
...
def _dump_panic(label, batch_no, err, cases, path):
    ...
    with open(p, "w") as f:
        ...
        f.write("\n\n--- cases ---\n")
        for c in cases:
            f.write("%s  subject=%r  nops=%d\n" % (c["id"], c["subject"], len(c["ops"])))
